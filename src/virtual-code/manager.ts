...
export class VirtualCodeCacheManager {
  ...
  /**
   * When true, the manager treats the on-disk cache as authoritative and
   * read-only. This is set by `loadFromDisk` and consumed by callers to
   * skip per-parse hash/mtime checks and virtual-file writes.
   */
  ...
  /**
   * Load an already-prepared cache from disk without rescanning or writing.
   * Used in "prepared" virtual-code mode where the cache is pre-generated by
   * the `svelte-eslint-parser-sync` CLI before lint runs.
   *
   * Returns true if a valid cache was found and loaded. Returns false if there
   * is no usable cache, in which case callers may fall back to full initialize().
   */
  public loadFromDisk(
    filePath: string,
    parserOptions: NormalizedParserOptions,
    ...
    const hashMap = loadHashMap(cacheDir, PARSER_VERSION);
    if (hashMap.size === 0) {
      return false;
    }
    ...
    this.generatedTsconfigPath = generatedTsconfigPath;

    // Resolve original tsconfig for path alias info used by import-rewriter
    // (this is read-only here, no scanning).
    const originalTsconfigPath = this.findOriginalTsconfig(parserOptions);
    if (originalTsconfigPath) {
      this.tsConfigInfo = readTsConfigWithExtends(originalTsconfigPath);
    }
  ...
  /**
   * Whether the cache is being treated as read-only (prepared mode).
   * Callers should skip per-parse hash/mtime checks and virtual-file writes
   * when this returns true — the on-disk cache is authoritative.
   */
